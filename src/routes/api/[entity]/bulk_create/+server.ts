...
import { EntityTypes, type EntityType, type EntityName, bulk_create } from "$lib/entities";
...
export const POST: RequestHandler = async ({ request, cookies, params }): Promise<Response> => {
  ...
  const result = await bulk_create<EntityType<typeof entityName>>(entityName, data, session);
